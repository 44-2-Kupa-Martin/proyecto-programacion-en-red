...
public abstract class Tile implements Disposable {
	...
	public <T extends Tile> Tile(World world, LayerId layerId, TileId tileId, int x, int y, Class<T> subclass) {
		...
		if (Constants.DEBUG) {
			...
			assert correspondingTileId != null : "The " + subclass.getSimpleName() + " class doesnt have a corresponding entry in the Tile.TileId enum";
			assert correspondingTileId == tileId : "The provided tileId " + tileId.name() + " is not the correct id for class " + subclass.getSimpleName();
		}
		...
		assert world.tiledMap.getLayers().get(LayerId.WORLD.value) instanceof TiledMapTileLayer : "Cannot cast!";
		...
		bodyDefiniton.position.set(Drop.tlToMt(x) - Constants.WORLD_WIDTH_mt / 2 + Drop.tlToMt(1) / 2,
				Drop.tlToMt(y) - Constants.WORLD_HEIGHT_mt / 2 + Drop.tlToMt(1) / 2);
	 ...
	 * api that accurately reflects the high level of coupling the {@link World} and {@code Tile}
	 ...
	 * 
	 * @param <T> The tile the specific implementation defines. This must be a parameter of the class
	...
	/**
	 * Enforces the requirement for all subclasses of tile to have a matching entry in the {@link TileId} enum
	...
	protected static final <T extends Tile> TileId debug_findCorrespondingId(Class<T> subclass) {
		assert Constants.DEBUG : "Debug only method " + Thread.currentThread().getStackTrace()[1].getMethodName() + " is called outside of debug build!";
		if (!Constants.DEBUG) 
			throw new RuntimeException("Debug only method " + Thread.currentThread().getStackTrace()[1].getMethodName() + " is called outside of debug build!");
	...
	public static enum TileId {
		// The constants' names must be the same as their corresponding class' name with added underscores for word separation, and they must be in uppercase
